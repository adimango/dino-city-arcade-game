...
export default class MainMenuScene extends Phaser.Scene {
  ...
  create() {
    ...
    const title = this.add
      .text(this.cameras.main.centerX, titleY, 'LA CITTÀ DEI DINOS', {
        fontSize: titleSize,
    ...
    const subtitle = this.add
      .text(this.cameras.main.centerX, subtitleY, 'ARCADE', {
    ...
    const highScoreText = this.add
      .text(
        ...
        highScoreY,
        `RECORD: ${window.gameState.highScore}`,
        {
          fontSize: buttonSize,
          fontFamily: 'Courier New, monospace',
          fill: '#7fb069',
    ...
    // Button border
    const startButtonBorder = this.add
      .rectangle(this.cameras.main.centerX, buttonY, buttonWidth, buttonHeight)
      .setStrokeStyle(4, 0x2d4a36)
      .setFillStyle();

    const startText = this.add
      .text(this.cameras.main.centerX, buttonY, 'INIZIA', {
        fontSize: buttonSize,
        fontFamily: 'Courier New, monospace',
        fill: '#000000',
      })
    ...
    let instructionText =
      'Clicca sui dinosauri per bloccarli!\nNon lasciarne scappare troppi!';
    if (isLandscape && isMobile) {
      instructionText = 'Tocca i dinosauri per bloccarli!';
    }

    const instructions = this.add
      .text(this.cameras.main.centerX, instructionsY, instructionText, {
    ...
    const soundButton = this.add
      .rectangle(
        ...
        soundButtonSize,
        40,
        window.gameState.soundEnabled ? 0x7fb069 : 0x666666
      )
      .setInteractive()
      .on('pointerdown', () => this.toggleSound(soundButton, soundText));

    // Sound button border
    const soundButtonBorder = this.add
      .rectangle(soundX, soundY, soundButtonSize, 40)
    ...
    const soundText = this.add
      .text(
        soundX,
        soundY,
        window.gameState.soundEnabled ? 'SUONI ON' : 'SUONI OFF',
        {
  ...
  createCitySilhouette() {
    ...
    for (let i = 0; i < numBuildings; i++) {
      ...
      if (Math.random() < 0.5) {
        // Damaged roof
        const damage = this.add.triangle(
          x + buildingWidth / 2,
          y,
          0,
          0,
  ...
  startBackgroundMusic() {
    ...
    if (
      ...
      !this.registry.get('backgroundMusicPlaying')
    ) {
  ...
  toggleSound(button, text) {
    ...
    // Track sound toggle
    track('sound_toggled', {
      enabled: window.gameState.soundEnabled,
    });
  ...
  startGame() {
    // Track game start
    track('game_started', {
      highScore: window.gameState.highScore,
